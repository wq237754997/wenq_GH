        ...
        return this.each(function() { // 对每个元素进行操作
            ...
            that.find(options.tabBtns).each(function(i, n) {
                $(n).bind(options.event, function() {
                    ...
                    that.find(options.tabUns).eq(i).addClass(options.hover).siblings().removeClass(options.hover); // z-index切换
                });
            });

            that.find(options.tabBtns).eq(0).addClass(options.currentStyle);
            that.find(options.tabUns).eq(0).addClass(options.hover);

            var maxNum = that.find(options.tabBtns).size();
            ...
            function core() {
                ...
                that.find(options.tabUns).eq(num).addClass(options.hover).siblings().removeClass(options.hover); // z-index切换
            };
